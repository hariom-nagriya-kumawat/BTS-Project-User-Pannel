...
import { hideLoader, cartAction, getCartSuccess } from "../../actions";
let toastId = null;
    ...
    if (result.isError) {
      ...
      let obj = {
        ...
        item: {
          ...
          online_price: action.payload.price,
          _id: action.payload.item_id,
        },
      };
      dispatch(
        getCartSuccess({
  ...
  async process({ action }, dispatch, done) {
    ...
    if (token) {
      ...
      if (result.isError) {
        ...
        let viewCartData = localStorage.getItem("viewCartData");
        let data = JSON.parse(viewCartData);
        if (data && data.length) {
          dispatch(
            getCartSuccess({
...
const updateCartLogic = createLogic({
  ...
  async process({ action, getState }, dispatch, done) {
    ...
    if (localStorage.getItem("token") && action.payload.cart_id) {
      let id = action.payload.cart_id;
      delete action.payload.cart_id;
      ...
      if (result.isError) {
        ...
        if (result.data && result.data.is_removed) {
          let index = data.findIndex(
            (item) => item._id === action.payload.cart_id
          );
          data.splice(index, 1);
        } else {
          let index = data.findIndex(
      ...
      result[index] = {
        ...
        item: { ...result[index].item, online_price: action.payload.price },
      };
      data[index1] = {
        ...data[index1],
        quantity: action.payload.quantity,
        item: { ...data[index1].item, online_price: action.payload.price },
      };
